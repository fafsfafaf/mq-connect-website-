    ...
    <section ref={sectionRef} className="py-24 relative overflow-hidden bg-white content-visibility-auto">
      ...
      <div className="relative z-10 w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-24 items-center">
          ...
          {/* Right Column: Active Video Card (Simplified) */}
          <div className="flex flex-col items-center">
            {/* Aspect Ratio Container */}
            <div className="relative w-[300px] sm:w-[320px] aspect-[9/16] rounded-[2rem] shadow-2xl bg-white border-4 border-white overflow-hidden z-10">
              {activeItem.type === 'video' ? (
                <div className="w-full h-full bg-slate-900 relative">
                  {/* Render Player only when visible to save resources */}
                  {isInView ? (
                    <YouTubePlayer
                      // Key forces re-mount on change -> instant simple switch, no heavy animation
                      key={activeItem.data.videoId}
                      videoId={activeItem.data.videoId}
                      isMuted={isMuted}
                      onEnded={handleNext}
                    />
                  ) : (
                    <img
                      src={`https://img.youtube.com/vi/${activeItem.data.videoId}/maxresdefault.jpg`}
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-6 text-left pointer-events-none z-20">
                    <div className="flex justify-between items-end">
                      <div>
                        ...
                        <h3 className="text-2xl font-bold text-white mb-1 leading-tight text-shadow-lg">{activeItem.data.title}</h3>
                        <p className="text-sm text-slate-100 font-medium tracking-wide text-shadow">{activeItem.data.subtitle}</p>
                      </div>
                ...
                <div className="w-full h-full bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400 p-1">
            ...
            <div className="flex lg:hidden items-center justify-center gap-4 mt-8 w-full max-w-[320px]">
