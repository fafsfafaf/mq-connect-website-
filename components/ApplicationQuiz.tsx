...
// Helper for option buttons to ensure consistent styling
...
export const ApplicationQuiz: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Lock body scroll on mobile only when overlay is open
  ...
  // Total steps including the new intro question
  const totalSteps = 8;
  const progressPercentage = (state.step / totalSteps) * 100;

  const handleAnswer = (key: keyof QuizState['answers'], value: any) => {
    setState(prev => ({
      ...prev,
  ...
  const nextStep = () => {
    ...
    // Age validation (now step 1)
    if (step === 1 && answers.age) { 
       if (answers.age < 18) {
         setState(prev => ({ ...prev, isDisqualified: true, disqualificationReason: 'age' }));
         return;
       }
    }
  ...
  const handleStartNo = () => {
    // If user clicks "Nein" on the first question, they are not interested
  ...
  const submitForm = async (e: React.FormEvent) => {
    ...
    // Immediate submission without animation delay
    setState(prev => ({ ...prev, isSubmitted: true }));
    setIsSubmitting(false);
  };

  const handleDisqualificationRedirect = () => {
    if (isOverlayOpen) {
  ...
  const renderDisqualificationModal = () => (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/90 animate-in fade-in duration-300">
      <div className="bg-white rounded-3xl w-full max-w-md p-8 text-center shadow-2xl scale-100 animate-in zoom-in-95 duration-300">
        ...
        <p className="text-slate-600 mb-8 leading-relaxed">
          {state.disqualificationReason === 'intent' 
        ...
        <Button fullWidth onClick={handleDisqualificationRedirect} size="lg">
  ...
  const renderPrivacyModal = () => (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/50" onClick={() => setShowPrivacy(false)}>
        <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                ...
                <button onClick={() => setShowPrivacy(false)} className="p-2 hover:bg-slate-200 rounded-full transition-colors">
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 text-right">
                <Button onClick={() => setShowPrivacy(false)} size="sm">Verstanden & Schließen</Button>
            </div>
        </div>
    </div>
  );
  ...
  const SuccessView = () => (
    ...
    >
      ...
      {/* Mobile close button on success - Fixed contrast issue */}
      {isMobile && isOverlayOpen && (
         <div className="mt-8">
            <Button 
              variant="secondary" 
  ...
  const renderStep = () => {
    switch (state.step) {
      case 0: // New Intro / Hook Question
        return (
            ...
            <div className="space-y-3">
              {['Muttersprachliches Niveau', 'Fließend in Wort & Schrift', 'Grundkenntnisse'].map(opt => (
                <OptionButton 
                  ...
                  onClick={() => { handleAnswer('germanLevel', opt); setTimeout(nextStep, 300); }}
                >
                  {opt}
                </OptionButton>
              ))}
              ...
              {[
                ...
                { label: 'Ja, Profi', sub: 'Ich weiß, wie man verkauft & abschließt' }
                 ...
                 <button 
                  ...
                  `}
                  onClick={() => { handleAnswer('experience', opt.label); setTimeout(nextStep, 300); }}
                >
                  <div>
            ...
            <div className="space-y-3">
              {['Sofort / Schnellstmöglich', 'In den nächsten 2-4 Wochen', 'Erst später'].map(opt => (
                <OptionButton 
                  ...
                  onClick={() => { handleAnswer('startDate', opt); setTimeout(nextStep, 300); }}
                >
                  {opt}
                </OptionButton>
              ))}
      ...
      case 8: // Contact Form
        return (
          <form onSubmit={submitForm} className="text-left space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            {/* Custom Checkbox - Fixed Visibility */}
            <label className="flex items-start gap-3 mt-4 cursor-pointer p-2 hover:bg-slate-50 rounded-lg transition-colors group">
              <div className="relative mt-0.5">
                  <input 
                    ...
                    type="checkbox" 
                    className="sr-only"
                    checked={state.answers.consent === true}
                    onChange={(e) => handleAnswer('consent', e.target.checked)}
                  />
                  <div className={`w-6 h-6 border-2 rounded transition-all flex items-center justify-center ${state.answers.consent ? 'border-[#004e82] bg-white' : 'border-slate-300 bg-white'}`}>
                      <Check
                        className={`w-4 h-4 text-black transition-all duration-200 ${state.answers.consent ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`} 
                        strokeWidth={3} 
                      />
                  </div>
              </div>
              <span className="text-xs text-slate-500 leading-relaxed pt-1 select-none">
                Ich stimme zu, dass meine Angaben zur Kontaktaufnahme gespeichert werden. Mehr Infos in der <button type="button" onClick={(e) => { e.preventDefault(); setShowPrivacy(true); }} className="underline text-[#004e82] hover:text-[#003d66] font-bold">Datenschutzerklärung</button>.
              </span>
            </label>
    ...
    <AnimatePresence mode="wait">
        {!state.isSubmitted ? (
          ...
          >
            ...
            {/* REFACTOR: Replaced backdrop-blur-sm with solid bg-white/95 */}
  ...
  // Mobile View: Full-screen overlay portal logic
  if (isMobile) {
    if (isOverlayOpen) {
       return createPortal(
        <div className="fixed inset-0 z-[9999] bg-slate-50 flex flex-col w-full h-[var(--vvh,100vh)]">
          ...
          {/* Inner scroll container for content to allow internal scrolling if content is taller than viewport */}
          <div className="w-full flex-grow overflow-y-auto flex flex-col justify-center px-4 py-8">
             <div className="w-full max-w-xl mx-auto my-auto pb-8">
                {state.isDisqualified && renderDisqualificationModal()}
       ...
       return (
          <Card className="bg-white border border-slate-200 shadow-lg p-6 sm:p-8 flex flex-col items-center text-center">
             ...
             <Button fullWidth onClick={() => setIsOverlayOpen(true)} size="lg" className="shadow-md">
  ...
  return (
    <div className="w-full max-w-xl mx-auto relative">
      {/* Disqualification Modal - Shows when disqualified */}
      {state.isDisqualified && renderDisqualificationModal()}
      {/* Privacy Modal */}
      {showPrivacy && renderPrivacyModal()}
      {/* Quiz Content */}
